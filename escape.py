# Space escape game
# credit to Sean McManus / www.sean.co.uk  
# Art by Rafael Pimenta

# Quick references for pygame 
...
for planetsectors in range(1, 26):  # Generate rooms 1-25 here
    GAME_MAP.append(["The dusty planet surface", 13, 13, True, True])
GAME_MAP += [
    # ['Room name', height, width, Top exit?, right exit?]  Rooms 26-50 and secret 51-55
    ...
    ["Secret Passageway", 9, 15, True, True],
    ["Secret Passageway", 9, 9, False, True],
    ["Secret Passageway", 9, 15, False, True],
    ["Secret Passageway", 9, 9, False, True],
    ["Secret Passageway", 9, 15, True, False]
]

# Quick Debug test to make sure that the map size and given map are the same
assert len(GAME_MAP) - 1 == MAP_SIZE, "MAP_SIZE and GAME_MAP do not match"
...
objects = {
    ...
    4: [images.bed, images.half_shadow, "A tiny and comfortable bed"],
    ...
    10: [images.cabinet, images.half_shadow, "A small locker, for personal items"],
    ...
    68: [images.food, None,
         "A food pouch. Use it to get 100% health.", "ready-to-eat food"],
...
for room in range(1, 26):  # add random scenery in planet locations
    if room != 13:  # skip 13
        ...
        scenery[room] = [
            [scenery_item, random.randint(2, 10), random.randint(2, 10)]]

# use loop to add fences to the planet surface zones.
...
def generate_map():
    ...
    # create top of room map
    room_map = [[side_edge] * room_width]
    # add middle sections(wall, floor to fill, wall)
    for y in range(room_height-2):
        room_map.append([side_edge] + [floor_type] *
                        (room_width - 2) + [side_edge])
    # create bottom of room
    ...
    if room_data[4]:  # if exit is on the right of this room
        room_map[middle_row][room_width - 1] = floor_type
        room_map[middle_row + 1][room_width - 1] = floor_type
        room_map[middle_row - 1][room_width - 1] = floor_type

    if current_room % MAP_WIDTH != 1:  # if the room is not on the left of map
        room_to_left = GAME_MAP[current_room - 1]
        # if a room is on the left and has a right exit, add left exit in this room
        if room_to_left[4]:
            room_map[middle_row][0] = floor_type
            room_map[middle_row + 1][0] = floor_type
            room_map[middle_row - 1][0] = floor_type

    if room_data[3]:  # if exit is on the top of this room
    ...
    if current_room < MAP_SIZE - MAP_WIDTH:  # if room is not on the bottom row
        ...
        # if room below has a top exit, add exit to the bottom of this room
        if room_below[3]:
...
def game_loop():
    ...
    if player_x == room_width:  # through the door on RIGHT
    ...
    if player_x == -1:  # through the door on LEFT
        clock.unschedule(hazard_move)
        current_room -= 1
        generate_map()
    ...
    if player_y == room_height:  # through the door on BOTTOM
        clock.unschedule(hazard_move)
    ...
    if player_y == -1:  # through the door on TOP
        clock.unschedule(hazard_move)
        current_room -= MAP_WIDTH
        generate_map()
    ...
    if keyboard.r and item_carrying:
    ...
    ## TESTING THE DOORS (test door in room 26 with pressure pad)
    ## DO NOT KEEP IN RUNNING GAME COMMENT OUT OR REMOVE
    # if keyboard.x:
    ...
    # if player is stnding somewhere they should'nt move them back
    if room_map[player_y][player_x] not in items_player_may_stand_on or hazard_map[player_y][player_x] != 0:
        player_x = old_player_x
        player_y = old_player_y
        player_frame = 0

    if room_map[player_y][player_x] == 48:  # if player is on a toxic floor spot
        deplete_health(1)
...
def draw():
    ...
    for y in range(room_height):
        for x in range(room_width):
            ...
            if hazard_here != 0:  # if hazzard is here
...
def show_text(text_to_shadow, line_number):
    if game_over:
        return
    text_lines = [15, 50]
    ...
    screen.draw.text(
        text_to_shadow, (20, text_lines[line_number]), color=GREEN)
...
# props that can appear or disappear.
# props must be set up here. Props not yet in the game go into room 0.
...
for key, prop in props.items():
    if key != 71:  # 71 is different each game SKIP
...
for recipe in RECIPES:
    checksum += (recipe[0] * check_counter + recipe[1] *
                 (check_counter + 1) + recipe[2] * (check_counter + 2))
    check_counter += 3
print(len(RECIPES), "recipes")
# assert len(RECIPES) == 11
print("Recipe checksum: ", checksum)
# assert checksum == 37296, "Error in recipe data"
...
def pick_up_object():
    ...
    if item_player_is_on in items_player_may_carry:
        ...
        show_text("Item in hands: " + objects[item_player_is_on][3], 0)
        sounds.pickup.play()
        time.sleep(0.5)
    else:
        show_text("You are not able to carry that", 0)


def add_object(item):  # add item to inventory
    global selected_item, item_carrying
    in_my_pockets.append(item)
    item_carrying = item
    selected_item = len(in_my_pockets) - 1
    display_inventory()
    props[item][0] = 0  # objects being carried go into room 0
...
def show_instructions():
    # Rect((left, top), (width, height))
    box = Rect((0, 200), (800, 105))
    screen.draw.filled_rect(box, WHITE)

    instructions = ['Escape before you run out of air or health!','E: Pick up objects', 'R: Drop Item in hand', 'Q: Interact with objects', 'Q: Stand over a dropped item and try to combine it with current item in hand', 'Spacebar: get details on something', 'Use arrow keys or WASD to move']
    for instruction in instructions:   
        screen.draw.text(instruction, (20, (15 * (instructions.index(instruction) + 1))), color="white")
...
def examine_object():
    ...
    if item_player_is_on in [0, 2]:  # won't describe the floor
        return
    description = "You are looking at: " + objects[item_player_is_on][2]
    for prop_number, details in props.items():
        # props = object number: [room number, y, x]
        if details[0] == current_room:  # if prop is in room
            #if prop is hidden ( = at player's location but not on map)
            if (details[1] == player_y and details[2] == left_tile_of_item and room_map[details[1]][details[2]] != prop_number):
                add_object(prop_number)
                description = "You hav found " + objects[prop_number][3]
                sounds.combine.play()
...
def use_object():
    global room_map, props, item_carrying, air, selected_item, health
    ...
    if item_carrying == 70 or item_player_is_on == 70:
        ...
        use_message = "AIR: " + str(air) + "% / HEALTH " + str(health) + "% / "
        ...
        use_message = "You use " + \
            objects[56][3] + " to repair the suit fabric"
        suit_stitched = True
        remove_object(58)
        ...
        if health > 100:
            health = 100
        use_message = "You munch the lettuce and get a little health back"
        draw_health_air()
        ...
        health = 100
        use_message = "You use the food to restore your health"
        remove_object(68)
        draw_health_air()
    ...
    for recipe in RECIPES:
        ...
        if (item_carrying == ingredient1 and item_player_is_on == ingredient2) or (item_carrying == ingredient2 and item_player_is_on == ingredient1):
            use_message = "You combine " + \
                objects[ingredient1][3] + objects[ingredient2][3] + \
...
def open_door(opening_door_number):
    ...
    door_frames = [images.door1, images.door2,
...
def close_door(closing_door_number):
    ...
    door_frames = [images.door4, images.door3,
                   images.door2, images.door1, images.door]
    ...
    #if player is in same row as doorway, they are in open doorway
    if player_y == props[door_object_number][1]:
        if player_y == 0:  # in top of doorway
...
def do_door_animation():
    ...
    if door_frame_number == 5:
        if door_frames[-1] == images.floor:
            ...
            #regenerate room map from props to put door in room if required
        generate_map()
    else:
...
def door_in_room_26():
    ...
    shadow_frames = [images.door_shadow, images.door1_shadow,
                     images.door2_shadow, images.door3_shadow, images.door4_shadow, None]
...
def draw_health_air():
    ...
    # clear the bar before writing new health and air
    screen.draw.filled_rect(box, BLACK)
    screen.draw.text("AIR", (20, 766), color=BLUE)
    screen.draw.text("HEALTH", (180, 766), color=RED)
...
def deplete_health(penalty):
    ...
    draw_health_air()
    if health < 1:
        end_the_game("You're out of health!")
...
def hazard_move():
    ...
    for hazard in current_room_hazards_list:
        ...
        if (hazard_y == player_y and hazard_x == player_x) or (hazard_y == from_player_y and hazard_x == from_player_x and player_frame > 0):
            sounds.ouch.play()
            deplete_health(10)
        ...
        if room_map[hazard_y][hazard_x] not in items_player_may_stand_on or hazard_map[hazard_y][hazard_x] != 0:
            hazard_should_bounce = True
...
#         gameDisplay.fill(pygame.Color("peachpuff"))
#         message_to_screen('Welcome to Space Escape', BLACK, -100, 'large')
#         message_to_screen(
#             'Escape before you run out of air or health!', BLACK, -30)
#         message_to_screen(
#             'E: Pick up objects', BLACK, 10)
#         message_to_screen(
#             'R: Drop Item in hand', BLACK, 50)
#         message_to_screen(
#             'Q: Interact with objects', BLACK, 90)
#         message_to_screen(
#             'Q: Stand over a dropped item and try to combine it with current item in hand', BLACK, 130)
#         message_to_screen(
#             'Spacebar: get details on something', BLACK, 170)
#         message_to_screen(
#             'Q: Stand over a dropped item and try to combine it with current item in hand', BLACK, 210)
#         message_to_screen(
#             'Press C to Start the game or Q to quit', BLACK, 270, 'large')
...
#     clock.schedule_unique(draw_health_air, 0.5)
#     clock.schedule_unique(alarm, 10)
#     # higher number below gives a longer time limit
